...
RegisterServerEvent("Boutique:UpdateLimitedVeh")
AddEventHandler("Boutique:UpdateLimitedVeh", function(vehicleT)
    LimitedV = vehicleT
end)
                ...
                if (current >= price) then
                    ...
                    if CASHOUT[xPlayer.identifier] ~= nil then 
                        if CASHOUT[xPlayer.identifier] + price >= 5000 then
                            ...
                            --xPlayer.showAdvancedNotification('Boite Mail', 'Boutique GreenLife', 'Il vous reste '..CASHOUT[xPlayer.identifier]..'/5000 points à utiliser\nAvant de toucher votre bonus fidélité !', 'CHAR_MP_FM_CONTACT', 2)
                        end
                    end
                    onAccepted();
                    ...
                    onRefused();
                    xPlayer.showNotification('Vous ne posséder pas les points nécessaires pour votre achat visité notre boutique.')
                end
            else
                onRefused();
            end
...
RegisterCommand('addPoints', function(source, args)
    if source == 0 then
        ...
        if (identifier['fivem']) then
            ...
            LiteMySQL:Insert('tebex_players_wallet', {
                identifiers = after,
                transaction = 'Ajout de GoodCoins via la console',
                price = 0,
                currency = 'Points',
                points = args[2],
            });
...
RegisterCommand('VipPoints', function(source, args)
    if source == 0 then
        local identifier = GetIdentifiers(args[1]);
        if (identifier['fivem']) then
            local Target = ESX.GetPlayerFromId(args[1])
           ...
           -- SeaLogs('https://discord.com/api/webhooks/1093171665250287626/yEsenRvJVO0KA77_Hf3pPIYSZM0_3OHHVwpCsMw6zGhDZ1ndfzGLy3bskmxyIgbtp2Ln', "Boutique - GiveCoins","**CONSOLE** vien de GIVE des Coins a :\n**License** : "..Target.."\nNombre de Coins : "..args[2].."", 56108)
            Target.showNotification('Vous avez reçu votre bonus de '..args[2].. ' GoodCoins grâce au Vip !')
        else
            print('LE JOUEUR N\'A LIER AUCUN COMPTE FIVEM')
        end
    end
...
RegisterCommand('tebex_paid', function(source, args, rC)
    if source == 0 then
        LiteMySQL:Insert('tebex_players_wallet', {
            identifiers = args[1],
            transaction = 'Achat Tebex de '..args[2].. ' GoodCoins',
            price = 0,
            currency = 'EUR',
            points = args[2]
        })
...
AddEventHandler('aBoutique:BuyVehicle', function(model, Vprice, label, vtype, srcs)
    if vtype == "lbv" then
        local xPlayer = ESX.GetPlayerFromId(srcs)
        for k,v in pairs(LimitedV) do
            if v.carName == model then
                if tonumber(Vprice) == tonumber(v.price) then
                    OnProcessCheckout(srcs, tonumber(Vprice), string.format("Achat de : %s", label), function()
                        ...
                        if vtype == "lbv" then
                            TriggerEvent("WithdrawVehicle", xPlayer.source, model)
                        end
                        xPlayer.showNotification("Vous avez acheter : " .. label .. " dans la boutique !")
                    ...
                    TriggerEvent("tF:Protect", source, "(Boutique:BuyVehicle)")
                end
            end
        end
    else
        if SecurityVehicles[model].model == nil then 
            DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat 2')
            SeaLogs('', "Un Cheateur à été detecté","**"..GetPlayerName(source).."** à essayé d'acheter ***"..label.."***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : ['..price..'] GoodCoins', 56108)
            return
        end
        
        if SecurityVehicles[model].model == model and SecurityVehicles[model].price == Vprice then
            local xPlayer = ESX.GetPlayerFromId(source)
            ...
            DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat 3')
        end
    end
end)
...
AddEventHandler('aBoutique:BuyVehiclePlane', function(model, label)
    ...
    if SecurityVehiclesPlane[model].model == model then
        ...
        DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat 4')
    end
end)
...
AddEventHandler('aBoutique:BuyVehicleBoat', function(model, label)
    if SecurityVehiclesBoat[model] == nil then 
        DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat 5')
        SeaLogs('', "Un Cheateur à été detecté","**"..GetPlayerName(source).."** à essayé d'acheter ***"..label.."***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : ['..price..'] GoodCoins', 56108)
        return
    end
    if SecurityVehiclesBoat[model].model == model then
        ...
        DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat 6')
    end
end)
...
AddEventHandler('aBoutique:Entreprise', function()
    ...
    OnProcessCheckout(source, 5000, string.format("Pack Entreprise ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Entreprise pour 5000 Coins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Entreprise***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [5000] GoodCoins', 56108)       
    end, function()
        return
    end)
end)
...
RegisterNetEvent('aBoutique:BuyWeapon')
AddEventHandler('aBoutique:BuyWeapon', function(weaponName, price, label)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    
    OnProcessCheckout(source, price, string.format("Achat d'une arme [%s]", fivem), function()
        xPlayer.addWeapon(weaponName, 250)
        xPlayer.showNotification('Vous avez acquis ~g~' .. label .. ' ~s~pour ~g~' .. price .. ' Coins.')
        SeaLogs('', "Boutique - Achat", "**" .. GetPlayerName(xPlayer.source) .. "** vient d'acheter une arme : ***" .. label .. "***\n**License** : " .. xPlayer.identifier .. "\nPrix de l'achat : [" .. price .. "] GoodCoins", 56108)
    end, function()
        xPlayer.showNotification('~r~Achat annulé ou échoué.')
        return
    end)
end)
...
AddEventHandler('aBoutique:Illegal', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 7000, string.format("Achat de Pack Illégal ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Illégal pour 7000 Coins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Illégal***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [7000] GoodCoins', 56108)       
    end, function()
        return
    end)
end)

RegisterNetEvent('aBoutique:IllegalOrga')
AddEventHandler('aBoutique:IllegalOrga', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 7000, string.format("Achat de Pack Illégal ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Illégal pour 7000 Coins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Illégal Orga***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [7000] GoodCoins', 56108)       
    end, function()
        return
    end)
end)

RegisterNetEvent('aBoutique:IllegalGang')
AddEventHandler('aBoutique:IllegalGang', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 5000, string.format("Achat de Pack Illégal ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Illégal pour 5000 Coins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Illégal Gang***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [5000] GoodCoins', 56108)       
    end, function()
        return
    end)
end)

RegisterNetEvent('aBoutique:CertifInsta')
AddEventHandler('aBoutique:CertifInsta', function(Username)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    local username = Username
    OnProcessCheckout(source, 1000, string.format("Achat Certification Instagram ["..fivem.."]"), function()
        MySQL.Async.execute("UPDATE phone_instagram_accounts SET verified = 1 WHERE username = '"..username.."'", {}, 
        function() end)  
        xPlayer.showNotification("Vous avez acheter : Certification Instagram dans la boutique !")
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Certification Instagram***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [1000] GoodCoins', 56108)       
    end, function()
        return
    end)
end)

RegisterNetEvent('aBoutique:CertifTwitter')
AddEventHandler('aBoutique:CertifTwitter', function(Username)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    local username = Username
    OnProcessCheckout(source, 1000, string.format("Achat Certification Twitter ["..fivem.."]"), function()
        MySQL.Async.execute("UPDATE phone_twitter_accounts SET verified = 1 WHERE username = '"..username.."'", {}, 
        function() end)  
        xPlayer.showNotification("Vous avez acheter : Certification Twitter dans la boutique !")
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Certification Twitter***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [1000] GoodCoins', 56108)       
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:BuyCustomMax', function(vehicle)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 1000, string.format("Achat de Pack Customisation ["..fivem.."]"), function()
        TriggerClientEvent('aBoutique:BuyCustomMaxClient', xPlayer.source, vehicle)
        xPlayer.showNotification('Vous avez acquis le Pack Customisation pour 1000 Coins')
...
AddEventHandler('aBoutique:CircuitDrogue', function(vehicle)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 5000, string.format("Achat Circuit de drogue personnalisé ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis un Circuit de drogue personnalisé our 5000 Coins\nFaites un ticket sur Discord')
...
AddEventHandler('aBoutique:IllegalWeapon', function(vehicle)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 50000, string.format("Achat Vente d\'arme Illégal ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis Vente d\'arme Illégal pour 50 000 Coins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Vente d\'arme Illégal***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [50000] GoodCoins', 56108)    
    end, function()
        return
    end)
end)

RegisterNetEvent('aBoutique:ChangePoint')
AddEventHandler('aBoutique:ChangePoint', function(vehicle)
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 2000, string.format("Achat déplacement gang ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis Changement de point Gang/Orga pour 2 000 Coins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Changement point orga***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [2000] GoodCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:entreprise', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 6000, string.format("Achat Entreprise ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez acquis une entreprise pour 6000 Coins\nFaites un ticket sur le Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Entreprise***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [6000] GoodCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:vehunique', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 7000, string.format("Véhicule au choix ["..fivem.."]"), function()
        xPlayer.showNotification('Vous avez un Véhicule au choix pour 7000 Coins\nFaites un ticket sur le Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Véhicule Unique***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [15000] GoodCoins', 56108)    
    end, function()
        return
    end)
end)
...
local WEAPON_CUSTOM_PRICE = {
    ...
    {name = 'WEAPON_MAZE', label = _U('weapon_maze'), components = {}},
            ...
            if CUSTOM then 
                if (CUSTOM.point ~= nil) then
                    OnProcessCheckout(source, CUSTOM.point, string.format("%s - %s", weaponName, customHash), function()
                        ...
                        xPlayer.showNotification('Vous avez acheter '..CUSTOM.label.. ' pour '..CUSTOM.point..' Coins')
                    end, function()
                        xPlayer.showNotification("Vous ne possédez pas les points nécessaires ("..CUSTOM.point.." GoodCoins)")
                    end)
                end
            end
        end
...
AddEventHandler('ewen:buyweapon', function(weapon, price, label)
    ...
    OnProcessCheckout(xPlayer.source, SecurityWeapons[weapon].price, string.format("Achat de : %s", label), function()
        ...
        xPlayer.showNotification("Vous avez acheter : " .. SecurityWeapons[weapon].label .. " dans la boutique !")
    end, function()
        xPlayer.showNotification("Vous ne possédez pas les points nécessaires.")
        return
    end)
...
RegisterNetEvent('GreenLife:process_checkout_case')
AddEventHandler('GreenLife:process_checkout_case', function(type)
    local source = source;
    if (source) then
        local identifier = GetIdentifiers(source);
        local xPlayer = ESX.GetPlayerFromId(source)
                    ...
                    ["helico"] = function(_s, license, player)
                        ...
                        MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE (`vehicle` LIKE @hash AND `type` = 'aircraft' AND `owner` = @owner) ", {
                            ...
                            if resultVeh[1] then
                                ...
                                local pointdepd = 0
                                if type == 'caisse_debutant' then
                                    pointdepd = 1000
                                elseif type == 'caisse_amateur' then
                                    pointdepd = 2000
...
function SeaLogs(webhook, name, message, color)
    ...
    local webhook = 'https://discord.com/api/webhooks/1226449689092554832/qDn5erxtjrjYehpZ0XKYjHFENdQDr03nkjcjWtnQPY8ITKm1n_zb4ps03B86w0CSg7NJ'
...
RegisterNetEvent('eBoutique:BuyPremium')
AddEventHandler('eBoutique:BuyPremium', function(type)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if (identifier['fivem']) then
        ...
        OnProcessCheckout(source, 1000, string.format("Achat de : GreenLifePass 1 Mois"), function()
            xPlayer.showNotification("Vous avez acheter : GreenLifePass (1 mois) sur la boutique !")
            SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***GreenLifePass (1 mois)***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [1000] GoodCoins', 56108)
            MySQL.Async.execute("UPDATE GreenLifePass SET premium = 1 WHERE citizenid = '"..citizenId.."'", {}, 
            function() end)  
        end, function()
            return
        end)
    ...
    if type == 'gold' then
        ...
        if (identifier['fivem']) then
            ...
            OnProcessCheckout(source, 1000, string.format("Achat de : VIP GOLD 1 Mois"), function()
                ...
                ExecuteCommand('VipPoints '..xPlayer.source..' 500')
                xPlayer.showNotification("Vous avez acheter : VIP ~g~GOLD (1 mois) sur la boutique !")
    ...
    elseif type == 'diamond' then
        ...
        if (identifier['fivem']) then
            ...
            OnProcessCheckout(source, 2000, string.format("Achat de : VIP Diamond 1 Mois"), function()
                ...
                xPlayer.showNotification("Vous avez acheter : VIP ~r~DIAMOND (1 mois) sur la boutique !")
        ...
        DropPlayer(source, 'Désynchronisation avec le serveur ou detection de Cheat 7')
    end
end)
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(MysteryBox.ListBox) do
        ESX.RegisterUsableItem(k, function(source)
            ...
            TriggerClientEvent('closeall', source)
            Wait(100)
            TriggerEvent('GreenLife:process_checkout_case_item', source, k, MysteryBox.ListBox[k].label)
        end)
    end
end)

RegisterNetEvent('GreenLife:process_checkout_case_item')
AddEventHandler('GreenLife:process_checkout_case_item', function(src, type, label)
    if (src) then
                    ...
                    ["helico"] = function(_s, license, player)
                        ...
                        MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE (`vehicle` LIKE @hash AND `type` = 'aircraft' AND `owner` = @owner) ", {
                            ...
                            if resultVeh[1] then
                                ...
                                if type == 'caisse_ruby2' then
                                    prixsalepd = 3000
                                    transactionpd = 'Remboursement (Caisse Ruby)'
                                elseif type == 'caisse_diamond2' then
                                    prixsalepd = 2000
                                    transactionpd = 'Remboursement (Caisse Diamond)'
                                elseif type == 'caisse_purple' then
                                    prixsalepd = 750
                                    transactionpd = 'Remboursement (Caisse Purple)'
                                else
                                    prixsalepd = 500
                                    transactionpd = 'Remboursement (Caisse Gold)'
                                end
