    ...
    if not Config.SqlKeybinding then
        ...
        favmenu.OnItemSelect = function(sender, item, index)
            ...
            if Config.FavKeybindEnabled then
                ...
                SimpleNotify("~g~" .. firstToUpper(FavoriteEmote) .. Translate('newsetemote'))
            end
        end
    end
    favEmotes = nil
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    if favEnabled and IsControlPressed(0, 21) then
                        if data.table ~= "Shared" then
                            ...
                            SimpleNotify("~g~" .. firstToUpper(data.name) .. Translate('newsetemote'))
                        else
...
function AddInfoMenu(menu)
    infomenu = _menuPool:AddSubMenu(menu, Translate('infoupdate'), "Les créateurs du menu emotes.", true, true)
