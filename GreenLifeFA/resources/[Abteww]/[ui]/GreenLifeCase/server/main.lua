...
ESX.RegisterServerCallback('GreenLifeCase:getPlayerDetails', function(source, cb)
...
ESX.RegisterServerCallback('GreenLifeCase:selectedCaseOpen', function(source, cb, caseData, itemData)
    local src = source
    ...
    if (identifier['fivem']) then
        ...
        MySQL.Async.fetchAll("SELECT SUM(points) FROM tebex_players_wallet WHERE identifiers = @identifiers", {
            ...
            result[1]["SUM(points)"] = tonumber(result[1]["SUM(points)"])
            caseData.price = tonumber(caseData.price)

            if (result[1]["SUM(points)"] >= caseData.price) then
                if (result[1]["SUM(points)"] ~= nil) then
                    cb(result[1]["SUM(points)"] - caseData.price)
                else
                    cb(0)
                end
                if result[1]["SUM(points)"] ~= nil then    
                    if caseData.priceType == "Coins" then 
                        if result[1]["SUM(points)"] >= caseData.price then
                            OnProcessCheckout(source, caseData.price, string.format("Achat caisse mystère"), function()
                            end, function()
                                return
                            end)
                        else
...
ESX.RegisterServerCallback('GreenLifeCase:collectItem', function(source, cb, itemData, caseData)  
    ...
    --     TriggerClientEvent('GreenLifeCase:serverNotif', -1, {firstName = firstName, lastName = lastName, itemLabel = itemLabel, itemImage = itemImage})
    -- end
...
ESX.RegisterServerCallback('GreenLifeCase:sellItem', function(source, cb, caseData, itemData)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local citizenId = xPlayer.identifier  
    if caseData.priceType == "Coins" then 
...
ESX.RegisterServerCallback('GreenLifeCase:sendInput', function(source, cb, data)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local citizenId = xPlayer.identifier
    local inputData = data.input
    local result = ExecuteSql("SELECT * FROM GreenLifeCase_codes WHERE code = '"..inputData.."'")
    if result[1] ~= nil then
        ExecuteSql("DELETE FROM GreenLifeCase_codes WHERE code = '"..inputData.."'")
        ExecuteSql("UPDATE GreenLifeCase SET goldcoin = goldcoin + '"..result[1].creditCount.."' WHERE citizenid = '"..citizenId.."'")
...
-- RegisterNetEvent('GreenLifeCase:addGoldCoin')
-- AddEventHandler('GreenLifeCase:addGoldCoin', function(amount)
--     local _source = source
--     local xPlayer = ESX.GetPlayerFromId(_source)
--     local citizenId = xPlayer.identifier
--     local deger = tonumber(amount)
--     ExecuteSql("UPDATE GreenLifeCase SET goldcoin = goldcoin + '"..deger.."' WHERE citizenid = '"..citizenId.."'")
--     -- SendToDiscord("CitizenID: ``"..citizenId.."``\n``"..deger.."``\n**Gold Coin ADDED!**")
-- end)

-- RegisterNetEvent('GreenLifeCase:addSilverCoin')
-- AddEventHandler('GreenLifeCase:addSilverCoin', function(amount)
--     local _source = source
--     local xPlayer = ESX.GetPlayerFromId(_source)
--     local citizenId = xPlayer.identifier
--     local deger = tonumber(amount)
--     ExecuteSql("UPDATE GreenLifeCase SET silvercoin = silvercoin + '"..deger.."' WHERE citizenid = '"..citizenId.."'")
--     SendToDiscord("CitizenID: ``"..citizenId.."``\n``"..deger.."``\n**Silver Coin ADDED!**")
-- end)
...
RegisterCommand('purchase_caseopening_credit', function(source, args)
    ...
    if src == 0 then
        ...
        inProgress = true
        local result = ExecuteSql("SELECT * FROM GreenLifeCase_codes WHERE code = '"..tbxid.."'")
        if result[1] == nil then
            ExecuteSql("INSERT INTO GreenLifeCase_codes (code, creditCount) VALUES ('"..tbxid.."', '"..credit.."')")
...
Discord_Webhook = "https://discord.com/api/webhooks/1226441899750068279/ScnwiaeAKIh07QN9cCOC4E8LnqSc4E-TRHnTYzH7YTF0FzXImx6Dk5r5_6wcyqv6gmHy"
function SendToDiscord(name, message, color)
	if Discord_Webhook == "https://discord.com/api/webhooks/1226441899750068279/ScnwiaeAKIh07QN9cCOC4E8LnqSc4E-TRHnTYzH7YTF0FzXImx6Dk5r5_6wcyqv6gmHy" then
	else
		local connect = {
            {
                ["color"] = color,
