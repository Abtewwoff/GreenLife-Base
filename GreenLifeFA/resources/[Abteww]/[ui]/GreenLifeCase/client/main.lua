...
AddEventHandler('GreenLifeCase:RefreshCoins', function(coins)
    if (coins) then
        SendNUIMessage({
            type = 'refreshCoins',
            myGoldCoin = coins,
        ...
        ESX.TriggerServerCallback("GreenLifeCase:getPlayerDetails", function(result)
            SendNUIMessage({
                type = 'refreshCoins',
                myGoldCoin = result,
            })	
...
RegisterNUICallback("collectItem", function(data, cb)
    if collectItemSpamProtect < GetGameTimer() then 
        ...
        if lastSelected.itemName == data.collectedItem.itemName then 
            ESX.TriggerServerCallback("GreenLifeCase:collectItem", function(result)
                SendNUIMessage({
                    type = 'setLastItems', 
    ...
    TriggerEvent('GreenLifeCase:RefreshCoins')
end)
...
RegisterNUICallback("sellItem", function(data, cb)
    if sellItemSpamProtect < GetGameTimer() then 
        ...
        if lastSelected.sellCredit == data.sellItem.sellCredit then 
            ESX.TriggerServerCallback("GreenLifeCase:sellItem", function(result)
                cb(result)
    ...
    if openCaseSpamProtect < GetGameTimer() then 
        ...
        if data then 
            ...
            if lastValidatedCase then 
                ...
                ESX.TriggerServerCallback("GreenLifeCase:selectedCaseOpen", function(result)
                    if result then
                        TriggerEvent('GreenLifeCase:RefreshCoins', result)
                        cb(lastSelected)
                    else
                        cb(false)
                    end
...
RegisterNUICallback('sendInput', function(data, cb)
	if sendInputSpamProtect <= GetGameTimer() then
		...
		ESX.TriggerServerCallback("GreenLifeCase:sendInput", function(result)
			if result then 	
				cb(tonumber(result))
			else
				cb(false)
...
RegisterNetEvent('GreenLifeCase:serverNotif')
AddEventHandler('GreenLifeCase:serverNotif', function(data)
	SendNUIMessage({
        type = 'serverNotif', 
        notifInfo = data,
    })	
...
--                 TriggerServerEvent('GreenLifeCase:addGoldCoin', AK4Y.PlayTimeRewardCoin)
--             else
--                 TriggerServerEvent('GreenLifeCase:addSilverCoin', AK4Y.PlayTimeRewardCoin) 
--             end
--         end
--     end
-- end)
