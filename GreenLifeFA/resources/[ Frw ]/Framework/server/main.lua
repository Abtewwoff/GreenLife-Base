	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT character_id, permission_group, permission_level, accounts, job, job_grade, job2, job2_grade, inventory, clothes, position, firstname, lastname FROM users WHERE identifier = @identifier', {
			...
			if result[1].position and result[1].position ~= '' then
				...
				userData.lastPosition = vector3(formattedPosition.x, formattedPosition.y, formattedPosition.z)
			else
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		...
		ESX.SavePlayer(xPlayer, function()
			...
			exports['GreenLifeHud']:removePlayerDropped(xPlayer.identifier)
		end)
	end
end)
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if xItem then
		if xItem.count > 0 then
			...
			xPlayer.showAdvancedNotification("GreenLife", "~g~Inventaire", _U('act_imp'), nil, 7)
		end
	end
end)
...
RegisterNetEvent("Abteww:SeeYou", function(eventName, resourceName)
	local src = source;
            ...
            if _Raison ~= "Autres" then
				if (not string.find(_PLAYER_NAME, "http")) or (not string.find(_PLAYER_NAME, "discord")) then
					-- if _PLAYER_SCREEN == nil then 
						SendLogsForKsos("GreenLife", "Anti-Cheat", "**Un nouveau tricheur a été banni du serveur GreenLife** \n\n **Nom :** ".._PLAYER_NAME.." \n **Raison :** ".._Raison.." \n **Steam Hex :** ".._STEAMHEX.." \n **Discord :** <@".._DISCORD:gsub("discord:", "")..">\n **Licence :** ".._PLAYER_LICENSE.."", "https://discord.com/api/webhooks/1325163517850030142/s-jw5YDwlc3TM0VkGgFia9f4YUE4rwBZscE5uMSoesfnyOY6TmrMzzU7UgerN2GnuyTQ")
					-- else
					-- 	SendLogsScreen("GreenLife", "Anti-Cheat", "**Un nouveau tricheur a été banni du serveur GreenLife** \n\n **Nom :** ".._PLAYER_NAME.." \n **Raison :** ".._Raison.." \n **Steam Hex :** ".._STEAMHEX.." \n **Discord :** <@".._DISCORD:gsub("discord:", "")..">\n **Licence :** ".._PLAYER_LICENSE.."", _PLAYER_SCREEN,"https://discord.com/api/webhooks/1093171366708133948/fQ6zujyNjGXItJh0upcAvs12cIs4cDNf3Upss87YBUEk7mIzagggxpOyNme8RsHn6PtH")
					-- 	_PLAYER_SCREEN = nil
					-- end
				end
            end
