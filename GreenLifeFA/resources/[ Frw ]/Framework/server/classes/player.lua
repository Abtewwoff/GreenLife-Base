...
function CreatePlayer(source, identifier, userData)
	...
	exports['GreenLifeHud']:LoadPlayerInventory(self.identifier, userData.inventory, userData.clothes, defaultSlots, Config.MaxWeight, self)
	...
	self.getInventory = function()
		return exports['GreenLifeHud']:getInventory(self.identifier)
	end
	self.getInventoryClothes = function()
		return exports['GreenLifeHud']:getInventoryClothes(self.identifier)
	end
	self.getInventoryItem = function(name)
		return exports['GreenLifeHud']:getInventoryItem(self.identifier, name)
	end

	self.saveInventory = function()
		return exports['GreenLifeHud']:saveInventory(self.identifier)
	end
	self.saveClothes = function()
		return exports['GreenLifeHud']:saveClothes(self.identifier)
	end
	...
	self.addInventoryItem = function(name, count, extra, bypass)
		...
		exports['GreenLifeHud']:addInventoryItem(self.identifier, name, count, extra, bypass)
	end
	...
	self.removeInventoryItemAtSlot = function(slot, count)
		...
		return exports['GreenLifeHud']:removeInventoryItemAtSlot(self.identifier, slot, count)
	end
	...
	self.removeInventoryItem = function(name, count, extra)
		...
		exports['GreenLifeHud']:removeInventoryItem(self.identifier, name, count, extra)
	end

	self.clearInventoryItem = function()
		exports['GreenLifeHud']:clearInventoryItem(self.identifier)
	end

	self.isFreeSlotOrSameItem = function(droppedTo, name)
		return exports['GreenLifeHud']:isFreeSlotOrSameItem(self.identifier, droppedTo, name)
	end
	self.addItemToSlot = function(droppedTo, count, seconditem)
		exports['GreenLifeHud']:addItemToSlot(self.identifier, droppedTo, count, seconditem)
	end
	...
	self.getWeight = function()
		return exports['GreenLifeHud']:getWeight(self.identifier)
	end
	self.getMaxWeight = function()
		return exports['GreenLifeHud']:getMaxWeight(self.identifier)
	end
	self.canCarryItem = function(name, count)
		return exports['GreenLifeHud']:canCarryItem(self.identifier, name, count)
	end
	...
	self.clearAllInventoryWeapons = function(perma)
		return exports['GreenLifeHud']:clearAllInventoryWeapons(self.identifier, perma)
	end
	...
	self.addWeapon = function(weaponName, ammo, addArgs, bypass)
		...
		if (exports['GreenLifeHud']:getWeaponIsPerma(weaponName)) then
		...
		exports['GreenLifeHud']:addInventoryItem(self.identifier, weaponName, 1, Data, bypass)
	end

	self.removeWeaponSystem = function(weaponName)
		if type(weaponName) ~= 'string' then return end

		exports['GreenLifeHud']:removeWeaponSystem(self.identifier, weaponName)
	end
	...
	self.getWeaponsById = function(id)
		return exports['GreenLifeHud']:getWeaponsById(self.identifier, id)
	end

	self.setWeaponsAmmoById = function(id, ammo)
		exports['GreenLifeHud']:setWeaponsAmmoById(self.identifier, id, ammo)
	end
	...
	self.getAccount = function(accountName)
		if (accountName == 'bank') then
			...
			return exports['GreenLifeHud']:getAccount(self.identifier, accountName)
		end
	end
		...
		if money > 0 then
			...
			if account then
				if (accountName == 'bank') then
					...
					exports['GreenLifeHud']:addInventoryItem(self.identifier, accountName, money)
				end
			end
		end
	end
		...
		if money > 0 then
			...
			if account then
				if (accountName == 'bank') then
					...
					exports['GreenLifeHud']:removeInventoryItem(self.identifier, accountName, money)
				end
			end
		end
	end
	...
	function self.updateVIP(callback)
		...
		MySQL.Async.fetchAll('SELECT * FROM tebex_accounts WHERE steam = @steam', {
			...
			if (result[1]) then
				...
				if (callback) then callback(self.vip) end
			end
		end);
	end
	...
	self.ban = function(time, message)
		...
		Print("[^3WARNING^7]  A changer en fonction de votre anti cheat")
	end
