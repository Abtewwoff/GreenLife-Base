		...
		RegisterCommand(commandName, function(source, commandArgs)
			...
			if command.group ~= nil then
				if (xPlayer and ESX.Groups[xPlayer.getGroup()].level >= ESX.Groups[command.group].level or (not xPlayer and source == 0)) then
					...
					if (xPlayer) then
						xPlayer.showNotification('~r~Permissions Insuffisantes !')
					end
				end
			else
				if (command.arguments > -1) and (command.arguments ~= #commandArgs) then
		...
		if tonumber(args[1]) and args[2] and tonumber(args[3]) then
			...
			if tPlayer then
				...
				if ESX.DoesJobExist(args[2], args[3]) then
					...
					SendLogsCommande("Setjob", "GreenLife | Setjob"," **"..SetjobeurName.."** vient de setjob **"..KargetName.."** en **"..args[2].."** "..args[3].." ", "https://discord.com/api/webhooks/1325162352370253946/ApTAotURnu2j15n6gw_oguJUqURKB9H28wyWZB7jBcwm2HyvABxbaXfZj38S0K1dXouB")
				else
		...
		if tonumber(args[1]) and args[2] and tonumber(args[3]) then
			...
			if xPlayer then
				...
				if ESX.DoesJobExist(args[2], args[3]) then
					...
					SendLogsCommande("Setjob2", "GreenLife | Setjob"," **"..SetjobeurNam2.."** vient de setjob2 **"..KargetName2.."** en **"..args[2].."** "..args[3].." ", "https://discord.com/api/webhooks/1325162473275265225/zKA7x_Bv971WgSS2QGpi1nVwe4WMloXtyq8cq1FEJsLlNjMBYR71BISEzx-GoCaBX_ju")
				else
					ESX.ChatMessage(source, 'That job does not exist.')
				end
			else
...
ESX.AddGroupCommand('car', 'animateur', function(source, args, user)
	...
	exports['GreenLifeHud']:SpawnVehicle(model, plyCoords, 0.0, nil, false, function(vehicle)
...
ESX.AddGroupCommand('giveitem', 'fondateur', function(source, args, user)
	...
	if xTarget then
		...
		if (count) then
			...
			xTarget.showNotification('Vous venez de recevoir x~g~'..count..' '..item..'~s~ dans vottre inventaire.')

			if (xPlayer) then
				xPlayer.showNotification('Le joueurs (~g~'..args[1]..' ~s~- ~g~'..XargetName..'~s~) viens de recevoir x~g~'..count..' '..item..'~s~ dans son inventaire.')
			end

		if args[2] == "cash" or args[2] == "dirtycash" then
			SendLogsCommande("Staff", "GreenLife | Give", "**"..ItemeurName.."** vient de donner "..count.." **"..item.."** a **"..XargetName.."**", "https://discord.com/api/webhooks/1325162656427937832/7nXJGqJ6gpl7jc0U-j3pKzzO_gmFZ0XmTzxiECDFunhZctAn-J3gq3_9KxIhzOxPjyKe")
		else
			SendLogsCommande("Staff", "GreenLife | Give", "**"..ItemeurName.."** vient de donner "..count.." **"..item.."** a **"..XargetName.."**", "https://discord.com/api/webhooks/1325162656427937832/7nXJGqJ6gpl7jc0U-j3pKzzO_gmFZ0XmTzxiECDFunhZctAn-J3gq3_9KxIhzOxPjyKe")
		end
...
ESX.AddGroupCommand('giveweapon', 'fondateur', function(source, args, user)
	...
	if (xPlayer) then
		...
		if (ESX.GetWeapon(weaponName)) then
			...
			xPlayer.showNotification('Vous venez de recevoir x~g~1 '..weaponName..'~s~ dans vottre inventaire.')

			if (xAdmin) then
				xPlayer.showNotification('Le joueurs (~g~'..args[1]..' ~s~- ~g~'..TargetName..'~s~) viens de recevoir x~g~1 '..weaponName..'~s~ dans son inventaire.')
			end

			SendLogsCommande("Staff", "GreenLife | Give", "**"..GiveurName.."** vient de donner **"..weaponName.."** a **"..TargetName.."**", "https://discord.com/api/webhooks/1325162753933049906/FNxZMjiS90s2xoJwRUByNm0ncvXK04GWDTtGUKTexUEOz56qzC4TpcRTXS5bdCXVm2f1")
		else
...
ESX.AddGroupCommand('clearinventory', 'admin', function(source, args, user)
	...
	if tPlayer then
		...
		SendLogsCommande("Clear", "GreenLife | Clear", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a clear l'inventaire de **"..tPlayer.name.."** (***"..tPlayer.identifier.."***)", "https://discord.com/api/webhooks/1325162857821765723/p0aApodUOr22ZEUGvXs9wghG3nK0Ok9hAO_6D2TmFgPapweS8DzQDypU06kIxvhFkYIv")
	else
		ESX.ChatMessage(source, "Le joueur n'est pas en ligne.")
	end
...
ESX.AddGroupCommand('clearallloadout', 'gerant', function(source, args, user)
	...
	if (tPlayer) then
		...
		SendLogsCommande("Clear", "GreenLife | Clear (PermaWeapon)", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a clear les armes de **"..tPlayer.name.."** (***"..tPlayer.identifier.."***)", "https://discord.com/api/webhooks/1325162935504343121/zBb5NGD3SZxu8kNZszdlZpDjaFeTV_uv-YOOC0VfgAXST1_xVdsU6Kl69QMz1XICgMdv")
	else
		ESX.ChatMessage(source, "Le joueur n'est pas en ligne.")
	end
...
ESX.AddGroupCommand('clearloadout', 'gerant', function(source, args, user)
	...
	if (tPlayer) then
        ...
        xPlayer.showNotification('Vous avez clear les armes de : '..tPlayer.name)
		SendLogsCommande("Clear", "GreenLife | Clear", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a clear les armes de **"..tPlayer.name.."** (***"..tPlayer.identifier.."***)", "https://discord.com/api/webhooks/1325163084108664832/c9B3fWvHbGd2AjE2ZKUPOmvBJNayveRqxB1FpZa_vQb6cZwwjF7XL-oUXtJJRHFcTS7N")
	else
		ESX.ChatMessage(source, "Le joueur n'est pas en ligne.")
	end
