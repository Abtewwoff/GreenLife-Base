...
function ESX.ShowNotification(msg, flash, saveToBrief, hudColorIndex, title, subject, icon,time,location)
    if title == nil then title = "GreenLife" end
    ...
    -- exports.iNotificationV3:Notification(title,subject,msg,icon,15,"left")

	exports['GreenLifeHud']:onAddNotification(subject, msg, 'info', time)
end 

function ESX.ShowAdvancedNotification(title, subject, msg,icon,time ,location)
    if title == nil then title = "GreenLife" end
    if subject == nil then subject = "Notification" end
	if msg == nil then msg = "Le message n'a pas pus être chargé, Contactez les développeur" end 
    if icon == nil then icon = "MESSAGE" end
	if time == nil then time = 15 end
	if location == nil then location = "left" end
    -- exports.iNotificationV3:Notification(title, subject, msg, icon, time, location)
...
function ESX.ShowHelpNotification(msg,time,location)
	...
	if location == nil then location = "left" end
	-- exports.iNotificationV3:HelpNotification( nil, nil, msg, time, location)

	exports['GreenLifeHud']:onSetHelpNotif(msg)
end 
