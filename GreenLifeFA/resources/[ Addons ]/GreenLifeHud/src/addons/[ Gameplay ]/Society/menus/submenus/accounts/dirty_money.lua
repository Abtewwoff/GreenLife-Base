...
function menu:LoadMenu(parent)
    ...
    menu.Menu:IsVisible(function(Items)
        Items:Separator(string.format("Argent disponible: %s~g~$~s~", MOD_Society.data?.dirty_money));
        ...
        Items:Button("Déposer de l'argent sale", nil, {
            ...
            onSelected = function()
                ...
                if (API_Player:InputIsValid(amount, "number") and tonumber(amount) > 0) then
                    ...
                    if (newAmount > 0) then
                        TriggerServerEvent('GreenLife:Society:AddMoney', MOD_Society.data.name, newAmount, "dirty_money")
                    else
                        ESX.ShowNotification("~r~Entré invalide.")
                    end
                else
                    ESX.ShowNotification("~r~Entré invalide.")
                end
        ...
        Items:Button("Retirer de l'argent sale", nil, {
            ...
            onSelected = function()
                ...
                if (API_Player:InputIsValid(amount, "number") and tonumber(amount) > 0) then
                    ...
                    if (newAmount > 0) then
                        TriggerServerEvent('GreenLife:Society:RemoveMoney', MOD_Society.data.name, newAmount, "dirty_money")
                    else
                        ESX.ShowNotification("~r~Entré invalide.")
                    end
