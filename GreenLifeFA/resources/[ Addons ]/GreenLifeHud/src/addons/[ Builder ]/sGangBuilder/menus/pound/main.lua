...
MOD_GangBuilder.Menus.MainPound:IsVisible(function(Items)
    ...
    if (vehicles) then
        if (GetNumberOfPoundVehicle(vehicles) > 0) then
            ...
            for vehiclePlate, vehicle in pairs(vehicles) do
                ...
                if (vehicle) then
                    ...
                    if (vehicle.stored == 0) then
                        ...
                        Items:Button(("%s ~c~[%s%s~c~]~s~"):format(vehicleName, "~b~", vehiclePlate), nil, {

                            RightBadge = RageUI.BadgeStyle.Car,
                            RightLabel = ("~g~$%s~s~"):format(GreenLife.enums.Pound.Prices["SpawnVehicle"]),
                            ...
                            onSelected = function()
                                ...
                                if (MOD_Vehicle:IsSpawnPointClear(coordsSpawn, 2)) then
                                    TriggerServerEvent('GreenLife:GangBuilder:PoundTakeVehicle', vehiclePlate, coordsSpawn)
                                    RageUI.CloseAll()
                                else
            ...
            Items:Separator()
            Items:Separator("~r~Aucun véhicule dans la fourrière.")
            Items:Separator()
        end
    else
