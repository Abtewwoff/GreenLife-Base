...
function menu:LoadMenu(parent)
    ...
    local AccesList = GreenLife.enums.GangBuilder.GradeAccesShow
    ...
    menu.Menu:IsVisible(function(Items)
        ...
        Items:Button("Suprimer", nil, { RightLabel = "→→→" }, true, {
            onSelected = function()
                TriggerServerEvent('GreenLife:GangBuilder:RemoveGrade', menu.DataMenu.name, MOD_GangBuilder.data.infos.id)
                RageUI.GoBack()
            end
        });
            ...
            if (acces.separator) then
                ...
                Items:Button(acces.label, nil, { RightLabel = Cheked }, true, {
                    onSelected = function()
                        ...
                        TriggerServerEvent('GreenLife:GangBuilder:ChangeStateGrade', menu.DataMenu.name, acces.name, menu.DataMenu.grade[acces.name], MOD_GangBuilder.data.infos.id)
                    end
                });
            end
        end
