...
function menu:LoadMenu(parent)
    ...
    menu.Menu:IsVisible(function(Items)
        ...
        Items:Button("Se téléporter (Menu Boss)", nil, { }, true, {
            onSelected = function()
                TriggerServerEvent('GreenLife:GangBuilder:GotoMenuBoss', menu.DataMenu.id)

                RageUI.CloseAll()
            end
        });
        Items:Button("Suprimer", nil, { }, true, {
            onSelected = function()
                TriggerServerEvent('GreenLife:GangBuilder:DeleteGang', menu.DataMenu.id)
