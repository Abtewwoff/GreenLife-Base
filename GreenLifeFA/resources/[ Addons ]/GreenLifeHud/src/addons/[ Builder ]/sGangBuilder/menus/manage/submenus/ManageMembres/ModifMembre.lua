...
function menu:LoadMenu(parent)
    ...
    menu.Menu:IsVisible(function(Items)
        ...
        Items:Button("Virer le membre", nil, { RightLabel = "→→→" }, true, {
            onSelected = function()
                TriggerServerEvent('GreenLife:GangBuilder:RemoveMembre', MOD_GangBuilder.data.infos.id, menu.DataMenu.license)
                RageUI.GoBack()
            end
        });
        ...
        for name, grade in pairs(MOD_GangBuilder.data.grades) do
            Items:Button(grade.label, 'Appuyez sur entrer pour sélectionner cet accès', { RightLabel = "→→→" }, true, {
                onSelected = function()
                    TriggerServerEvent('GreenLife:GangBuilder:ChangeMemberAcces', menu.DataMenu.license, name, MOD_GangBuilder.data.infos.id)
                    RageUI.GoBack()
                end
            });
        end
