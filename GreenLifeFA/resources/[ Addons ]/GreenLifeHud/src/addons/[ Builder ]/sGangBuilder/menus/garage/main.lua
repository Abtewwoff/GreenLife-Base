        ...
        if (SizeOf(vehicles) > 0) then
            ...
            for plate, vehicle in pairs(vehicles) do
                if (vehicle) then
                    ...
                    Items:Button(("%s ~c~[%s%s~c~]~s~"):format(vehicleName, "~b~", plate ), nil,
                    {
                        RightLabel = isStored and IsVehicleOwned(vehicle.owner) or "~r~SORTIE~s~",
                    }, isStored, {
                        onSelected = function()
            ...
            Items:Separator()
            Items:Separator("~r~Aucun véhicule.")
            Items:Separator()
        end
    end
...
RegisterNetEvent('GreenLife:GangBuilder:OpenGangGarage')
AddEventHandler('GreenLife:GangBuilder:OpenGangGarage', function(gangVehicles, gangInfos)
