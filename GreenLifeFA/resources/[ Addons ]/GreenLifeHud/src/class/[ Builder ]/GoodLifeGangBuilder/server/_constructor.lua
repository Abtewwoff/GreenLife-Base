_GreenLifeGangBuilder = {}

local __instance = {
    __index = _GreenLifeGangBuilder
}

setmetatable(_GreenLifeGangBuilder, {
    __call = function(_, newGang, data)
        ...
        self.societies = {
            name = self.name,
            label = self.label,
            grades = GreenLife.enums.GangBuilder.DefaultJobGrade
        }
        ...
        self.posGarage = MOD_Zones:AddZone(vector3(data.posGarage.x, data.posGarage.y, data.posGarage.z), function(xPlayer, Zone)
            ...
            TriggerClientEvent('GreenLife:GangBuilder:OpenGangGarage', xPlayer.source, self:GetVehiclesTemplate(), {
                name = self.name,
                label = self.label,
        ...
        self.posDeleteVeh = MOD_Zones:AddZone(vector3(data.posDeleteVeh.x, data.posDeleteVeh.y, data.posDeleteVeh.z), function(xPlayer, Zone)
            ...
            TriggerClientEvent("GreenLife:GangBuilder:GetParkVehicule", xPlayer.source, self.id)
        ...
        self.posCoffre = MOD_Zones:AddZone(vector3(data.posCoffre.x, data.posCoffre.y, data.posCoffre.z), function(xPlayer, Zone)
            ...
            TriggerEvent("GreenLife:Inventory:OpenSecondInventory", "coffregang", self.name, xPlayer)
        ...
        self.posBoss = MOD_Zones:AddZone(vector3(data.posBoss.x, data.posBoss.y, data.posBoss.z), function(xPlayer, Zone)
            ...
            TriggerClientEvent('GreenLife:GangBuilder:OpenGangManage', xPlayer.source, {
                name = self.name,
                label = self.label,
                spawnPos =  self.posSpawnVeh,
                id = self.id
		...
		self.maxWeight = GreenLife.enums.GangBuilder.MaxWeight;
		self.maxSlots = GreenLife.enums.GangBuilder.MaxSlots;
        ...
        --Functions
        exportMetatable(_GreenLifeGangBuilder, self)
