function _GreenLifeDrugWeed:UpgradePlantPotWeed(plotSize, plotIndex, potIndex, ProgressGrow)
    if (self.plotList[plotSize][plotIndex] == "empty") then
        return
    end

    local PotProcces = GreenLife.enums.Labos.Types['weed']['procces']['potRecolt']
    local PotInfos = GreenLife.enums.Labos.Types['weed']['plotManagement'].PlotList[plotSize][plotIndex].PotList[potIndex]

    local VarietySeed = GreenLife.enums.Labos.Types['weed']['procces'].Variety[self.plotList[plotSize][plotIndex].PotList[potIndex].metadata.variety]
