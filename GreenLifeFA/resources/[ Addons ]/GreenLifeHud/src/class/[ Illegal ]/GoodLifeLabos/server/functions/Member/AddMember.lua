function _GreenLifeLabos:AddMember(targetId)
    ...
    if (targetPlayer) then
        if (self.memberList[targetPlayer.identifier]) then
            ...
            for _, src in pairs(PlayersInBucket) do
                TriggerClientEvent('GreenLife:Labos:AddMember', src, targetPlayer.identifier, self.memberList[targetPlayer.identifier])
            end
        end
    end
end