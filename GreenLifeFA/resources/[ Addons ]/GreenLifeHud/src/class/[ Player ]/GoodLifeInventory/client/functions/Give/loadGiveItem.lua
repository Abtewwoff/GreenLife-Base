function _GreenLifeInventory:loadGiveItem(data)
            ...
            if (self.CurrentGiveItem) then
                ...
                if (IsControlJustPressed(1, 73)) then
                    if (Target == PlayerPedId()) then
                        ESX.ShowNotification('~r~Vous ne pouvez pas donner un objet a vous même !')
                        return
                    end

                    TriggerServerEvent('GreenLife:Inventory:GiveItemToPlayer', GetPlayerServerId(IdTarget), data)
                ...
                if (ClosestPlayer == -1 or ClosestPlayerDistance > 3.0) then
                    ESX.ShowNotification('~r~Il n\'y a aucun joueur autour de vous !')
                    self.CurrentGiveItem = false
                    self.GiveItemTablePlayer = {}
                else
