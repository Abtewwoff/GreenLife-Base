function _GreenLifeInventory:moveWeaponsToInv(from, to, count)
    ...
    if (not fromItem or not fromItem.count or not toItem) then
        -- print('~g~[GreenLifeHud] ^1This item is not exist in the inventory. Event: moveWeaponsToInv. ^7')
    end
    ...
    if (not ItemInfo_FROM and fromItem ~= "empty") or (not ItemInfo_TO and toItem ~= "empty") then
        -- print('~g~[GreenLifeHud] ^1Item is not exist in the system. ^7')
        return
    end
