function _GreenLifeInventory:StartkeyUtils()
    ...
    function InvUseShort(index)
        ...
        for slot, item in pairs(Inventory) do
            if (slot == index) and (item ~= "empty") and (not exports['GreenLife']:checkIfPlayerIsInSafeZone()) then
                
                -- TriggerServerEvent('GreenLife:Inventory:InvUseItem', index, true, GetWeaponAmmoInv())
                TriggerServerEvent('GreenLife:Inventory:InvUseItem', index, true)
