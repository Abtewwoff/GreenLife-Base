function MOD_Zones:AddZone(coords, handler, requireJob, interactDistance, helpText, notSendCl)
    ...
    MOD_Zones.list[NewZoneId] = _GreenLifeZones(NewZoneId, coords, handler, requireJob, interactDistance, helpText, notSendCl)

    if (not MOD_Zones.list[NewZoneId].notSendCl) then
        TriggerClientEvent('GreenLife:Zones:AddZone', -1, MOD_Zones.list[NewZoneId]:minify())
    end
