local function removeFromTable(plate, xPlayer)
    ...
    xPlayer.showNotification("~g~Vous avez rangé votre véhicule.");
end

RegisterNetEvent('GreenLife:Garage:ParkVehicle')
AddEventHandler('GreenLife:Garage:ParkVehicle', function(vehiclePlate, properties)
    ...
    MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner = @owner AND plate = @plate", {
        ...
        if (result[1] ~= nil) then
            ...
            xPlayer.showNotification("~r~Vous ne posséder pas ce véhicule.")
        end
    end)
end)