...
RegisterNetEvent('GreenLife:Inventory:InvSkinClothesChange')
AddEventHandler('GreenLife:Inventory:InvSkinClothesChange', function(data, reset, animation, saveSkin)
    if (animation) then
    ...
    if (reset) then
        for key, index in pairs(data) do
            local ResetIndex = GreenLife.enums.Player.defaultClothes[key]
