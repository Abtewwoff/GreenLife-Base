RegisterNetEvent('GreenLife:Inventory:InvMoveToSecond')
AddEventHandler('GreenLife:Inventory:InvMoveToSecond', function(data, weaponSave)
            ...
            if (SecondInv:isFreeSlotOrSameItem(droppedTo, playeritem.name)) then
                ...
                if (RemoveSucces) then
                    ...
                    if (playeritem.id == xPlayer.get('currentWeapon') and playeritem.args ~= nil) then
                        ...
                        TriggerClientEvent('GreenLife:Inventory:WeaponSet', source, nil)
