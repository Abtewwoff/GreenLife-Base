...
function menu:LoadMenu(parent)
    ...
    local DefaultAccesList = GreenLife.enums.Labos.AccessList['default']

    menu.Menu:IsVisible(function(Items)
        local DrugAccesList = GreenLife.enums.Labos.AccessList[MOD_Labos.LabData.DrugType]

        Items:Button("Nom de l'acces", nil, { RightLabel = menu.DataMenu.name }, true, {
        });
        Items:Button("Suprimer", nil, { RightLabel = "→→→" }, true, {
            onSelected = function()
                TriggerServerEvent('GreenLife:Labos:RemoveAcces', menu.DataMenu.name)
                RageUI.GoBack()
            end
        });
        ...
        for accesName, data in pairs(DefaultAccesList) do
            ...
            Items:Button(data.label, nil, { RightLabel = Cheked }, true, {
                onSelected = function()
                    ...
                    TriggerServerEvent('GreenLife:Labos:ChangeStateAcces', menu.DataMenu.name, accesName, menu.DataMenu.acces[accesName])
                end
            });
        end
