...
function MOD_Weed:StartPlotTask()
    local PlotListSmall = GreenLife.enums.Labos.Types['weed']['plotManagement'].PlotList['small']
    local PlotListBig = GreenLife.enums.Labos.Types['weed']['plotManagement'].PlotList['big']
    ...
    while (MOD_Weed.LabData.playerInLab) do
        ...
        Interval = MOD_Weed:LoadFloatText(GreenLife.enums.Labos.Types['weed']['plotManagement'].PlotList)


        local PlotList = GreenLife.enums.Labos.Types['weed']['plotManagement'].PlotList
        local OnClosetPlant = false

        for sizePlot in pairs(PlotList) do

            local PlotListSize = GreenLife.enums.Labos.Types['weed']['plotManagement'].PlotList[sizePlot]
                    ...
                    for indexPot in pairs(PlotListSize[indexPlot].PotList) do
                        ...
                        if (PotIsEmpty ~= "empty") then
                            ...
                            if (dist < 1.2 and not MOD_Weed.LabData.onUseItem) then
                                ...
                                SetEntityDrawOutlineColor(30, 255, 0, 255)

                                local PotProcces = GreenLife.enums.Labos.Types['weed']['procces'][PotIsEmpty.state]
                                if (PotProcces) then
