RegisterNetEvent('GreenLife:GangBuilder:ParkVehicule')
AddEventHandler('GreenLife:GangBuilder:ParkVehicule', function(plate, gangId, properties)
            ...
            MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE plate = @plate", {
                ...
                if (isOwned) then
                    if(result[1].boutique ~= nil and (result[1].boutique == 1 or result[1].boutique == true)) then
                        xPlayer.showNotification("~r~Ce véhicule provient de la boutique. Impossible de le stocker ici.")
                    else
                            ...
                            if (result[1].owner == xPlayer.getIdentifier()) then
                                MySQL.Async.fetchAll("DELETE FROM owned_vehicles WHERE plate=@plate AND owner=@owner", {
                                    ...
                                    if (Gang:AddVehicle(plate, properties, xPlayer.getIdentifier())) then
                                        ...
                                        Gang:UpdateEvent("GreenLife:GangBuilder:ReceiveVehicle", plate, Gang:GetVehicle(plate))
                                    end
                    ...
                    if (vehicle) then
                        ...
                        if (societyVehicle) then
                            ...
                            xPlayer.showNotification("~r~Ce véhicule n'appartient pas à votre société.")
                        end
