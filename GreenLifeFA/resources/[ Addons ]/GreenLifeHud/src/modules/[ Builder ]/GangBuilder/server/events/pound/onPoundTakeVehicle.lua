RegisterNetEvent('GreenLife:GangBuilder:PoundTakeVehicle')
AddEventHandler('GreenLife:GangBuilder:PoundTakeVehicle', function(vehiclePlate, spanwCoord)
            ...
            if (Gang) then
                if (Gang:DoesPlayerExist(xPlayer)) then
                    ...
                    if (not Gang:IsVehicleOut(vehiclePlate)) then
                        ...
                        local pricePound = GreenLife.enums.Pound.Prices['SpawnVehicle']
                            ...
                            if (playerMoney.money >= pricePound) then
                                ...
                                if (vehicleData) then
                                    MOD_Vehicle:CreateVehicle(vehicleData.data.model, vector3(spanwCoord.x, spanwCoord.y, spanwCoord.z), spanwCoord.h or 0.0, vehiclePlate, Gang.name, function(vehicle)
                                        ...
                                        Gang:UpdateEvent("GreenLife:GangBuilder:ReceiveVehicle", vehiclePlate, Gang:GetVehicle(vehiclePlate))
                
                                    end, xPlayer)
                                end
                            else
                                xPlayer.showNotification("~r~Vous n'avez pas assez d'argent pour sortir ce véhicule.")
                            end
                        end

                    else
                        xPlayer.showNotification("~r~Ce véhicule est indisponible, vous devez le retrouver là où vous l'avez laissé.")
                    end
