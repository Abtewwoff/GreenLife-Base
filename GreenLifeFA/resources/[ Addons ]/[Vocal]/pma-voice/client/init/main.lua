...
function playMicClicks(clickType)
    if micClicks ~= 'true' then print('HERE RETURN') return logger.verbose("Not playing mic clicks because client has them disabled") end
    -- TODO: Add customizable radio click volumes
    
    local sound = (clickType and "audio_on" or "audio_off")
    local volume = (clickType and 0.1 or 0.03)

    TriggerEvent('GreenLife:Hud:SetHudRadioSound', sound, volume)
end
...
function setVoiceProperty(type, value)
    if type == "radioEnabled" then
        ...
        TriggerEvent('GreenLife:Hud:SetHudRadioUse', value)
