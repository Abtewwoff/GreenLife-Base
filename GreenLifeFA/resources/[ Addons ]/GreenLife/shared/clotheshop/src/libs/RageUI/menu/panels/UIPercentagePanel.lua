...
function RageUIClothes.PercentagePanel(Percent, HeaderText, MinText, MaxText, Action, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RenderRectangle(CurrentMenu.X, CurrentMenu.Y + 5 + Percentage.Background.Y + CurrentMenu.SubtitleHeight + RageUIClothes.ItemOffset, Percentage.Background.Width + CurrentMenu.WidthOffset, Percentage.Background.Height, 20, 24, 28, 255)

            RenderRectangle(CurrentMenu.X + Percentage.Bar.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + Percentage.Bar.Y + CurrentMenu.SubtitleHeight + RageUIClothes.ItemOffset, Percentage.Bar.Width, Percentage.Bar.Height, 35, 39, 47, 255)
            RenderRectangle(CurrentMenu.X + Percentage.Bar.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + Percentage.Bar.Y + CurrentMenu.SubtitleHeight + RageUIClothes.ItemOffset, Progress, Percentage.Bar.Height, 30, 255, 0, 255)
