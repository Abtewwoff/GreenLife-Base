...
local level = 20;
local msg = "Votre garde-robe est pleine, veuillez vous procurer le VIP ~y~Mini~w~ ou Extreme~w~ pour l'agrandir."
...
RegisterNetEvent('shops:setStatVip', function(call)
    ...
    elseif (call == 2) then
        level = 25;
        msg = "Votre garde-robe est pleine, veuillez vous procurer le VIP Extreme~w~ pour l'agrandir."
    elseif (call == 3) then
        level = 30;
...
function Render:validChangeMenu()
    ...
    RageUIClothes.Line()
    RageUIClothes.Button('Oui', nil, {  RightLabel = "→→" }, true, {
        ...
        onSelected = function()
            ...
            if (#_Razzway.Data < level) then 
                ...
                if (nameTenue == "" or nameTenue == nil) then
                    ESX.ShowAdvancedNotification('Notification', "Vêtements", "Vous devez attitré un nom à votre tenue pour pouvoir la sauvegarder", 'CHAR_BINCO', 8)
                else
                ...
                ESX.ShowAdvancedNotification('Notification', "Vêtements", msg, 'CHAR_BINCO', 8)
            end

        end
    })
    RageUIClothes.Button('Non', nil, {  RightLabel = "→→" }, true, {
        onActive = function() Uclothes:OnRenderCam() end,
        onSelected = function()
            RageUIClothes.CloseAll()
        end
