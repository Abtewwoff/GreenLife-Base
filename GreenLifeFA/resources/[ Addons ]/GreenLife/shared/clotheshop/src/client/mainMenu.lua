...
function _Client.open:clothesMenu()
    ...
    while clothesMenu do
        ...
        RageUIClothes.IsVisible(clothesMenu, function()
            -- RageUIClothes.Line()
            RageUIClothes.Button('Dressing', nil, {  RightLabel = "→→" } , true, {
                onActive = function() Uclothes:OnRenderCam() end,
                onSelected = function()
                    _Razzway:refreshData()
                end
            }, myDressing)
            RageUIClothes.Button('Parcourir les Vêtements', nil, {  RightLabel = "→→" }, true, {onActive = function() Uclothes:OnRenderCam() end}, changeMenu)
            RageUIClothes.Line()
            RageUIClothes.Button(('Acheter les Vêtements %s$'):format(_Config.clotheshop.price), nil, { Color = { BackgroundColor = { 30, 255, 0, 170 } },  RightLabel = "→→" }, true, {
                onActive = function() Uclothes:OnRenderCam() end,
                onSelected = function()
