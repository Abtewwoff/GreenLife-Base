    ...
    CreateThread(function()
        ...
        if FirstSpawn then
            ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                if skin == nil then
                    LoadCreator();
                    --ESX.ShowAdvancedNotification("Événement", "~r~Purge", "Le serveur est en mode ~r~PURGE cela veut dire qu'aucune regle RP sera respectée !", nil, 8)
                else
...
RegisterNetEvent("Abteww:RegisterPlayerIdentity", function()
    LoadCreator();
end);
