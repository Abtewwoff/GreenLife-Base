...
AddEventHandler(_Prefix..':identity:setIdentity', function(playerInfo)
    ...
    if (CreatorConfig.consoleLogs) then
        -- print(("Le joueur ^3%s^7 a créé son identité ^3-->^7 [ Prénom : ~g~%s^7 | Nom : ^6%s^7 | Taille : ^2%s^7 | Anniv : ^1%s^7 | Sexe : ^4%s^7 ]"):format(GetPlayerName(xPlayer.source), playerInfo.firstname, playerInfo.name, playerInfo.height, playerInfo.birthday, playerInfo.sex))
    end
...
ESX.AddGroupCommand("register", "admin", function(src, args, xPlayer)
    ...
    if (target) then
        ...
        SendLogs("Register", "GreenLife | Register", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a register l'identité de **"..target.name.."** (***"..target.identifier.."***)", "https://discord.com/api/webhooks/1226441899750068279/ScnwiaeAKIh07QN9cCOC4E8LnqSc4E-TRHnTYzH7YTF0FzXImx6Dk5r5_6wcyqv6gmHy")
        SetEntityCoords(GetPlayerPed(target.source), -774.95, 318.24, 195.86-0.9);
        target.triggerEvent("Abteww:RegisterPlayerIdentity");
    end
end, {
