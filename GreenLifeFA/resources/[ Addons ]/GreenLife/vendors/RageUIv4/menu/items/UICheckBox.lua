...
local function StyleCheckBox(Selected, Checked, Box, BoxSelect, OffSet)
    ...
    if Checked then
        RenderSprite(SettingsCheckbox.Dictionary, SettingsCheckbox.Textures[BoxSelect], CurrentMenu.X + SettingsCheckbox.X + CurrentMenu.WidthOffset - OffSet, CurrentMenu.Y + SettingsCheckbox.Y + CurrentMenu.SubtitleHeight + RageUIv4.ItemOffset, SettingsCheckbox.Width, SettingsCheckbox.Height)
    else
        RenderSprite(SettingsCheckbox.Dictionary, SettingsCheckbox.Textures[3], CurrentMenu.X + SettingsCheckbox.X + CurrentMenu.WidthOffset - OffSet, CurrentMenu.Y + SettingsCheckbox.Y + CurrentMenu.SubtitleHeight + RageUIv4.ItemOffset, SettingsCheckbox.Width, SettingsCheckbox.Height)
    end
end
...
function RageUIv4.Checkbox(Label, Description, Checked, Style, Actions)
    ---@type table
    local CurrentMenu = RageUIv4.CurrentMenu;
                ...
                if type(Style) == "table" then
                    if Style.Enabled == true or Style.Enabled == nil then
                        if Selected then
                            ...
                            RenderText(Label, CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUIv4.ItemOffset, 0, SettingsButton.Text.Scale, 104, 108, 114, 255)
                        end
                        if type(Style) == 'table' then
                        ...
                        ---@type number
                        local LeftBadgeOffset = ((LeftBadge == RageUIv4.BadgeStyle.None or LeftBadge == nil) and 0 or 27)

                        if Selected then
                            RenderText(Label, CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUIv4.ItemOffset, 8, SettingsButton.Text.Scale, 0, 0, 0, 255)
                        else
                    ...
                    if Style.Style ~= nil then
                        if Style.Style == RageUIv4.CheckboxStyle.Tick then
                            StyleCheckBox(Selected, Checked, 2, 4, BoxOffset)
                        elseif Style.Style == RageUIv4.CheckboxStyle.Cross then
...
function RageUIv4.ReloadAnimationCheckBox()
    Citizen.CreateThread(function() 
        while true do
            Wait(100)
            progressValue = 0
