...
RegisterNetEvent("tattoos:remove", function()
	...
	if (not playersTimeded[xPlayer.identifier] or GetGameTimer() - playersTimeded[xPlayer.identifier] > 10000) then
		...
		if xPlayer.getAccount('cash').money >= price then
			...
			TriggerClientEvent("esx:showNotification", xPlayer.source, "🧼 ~g~Tu viens de supprimer tes tatouages.")
			TriggerClientEvent("tattoos:getPlayerTattoos", xPlayer.source, {})
		else
...
AddEventHandler("tattoos:save", function(tattoosList, price, value)
	...
	if xPlayer.getAccount('cash').money >= price then
		...
		TriggerClientEvent("esx:showNotification", xPlayer.source, "🖊️ ~g~Tu viens de te faire un tatouage")
	else
		TriggerClientEvent("esx:showNotification", xPlayer.source, "💰 ~r~Vous n'avez pas assez d'argent.")
	end
end)
