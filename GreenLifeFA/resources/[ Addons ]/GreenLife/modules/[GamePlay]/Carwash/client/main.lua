...
RMenuv1.Add('wash', 'main', RageUIv1.CreateMenu("", "~g~Actions disponibles"))
		...
		CreateThread(function()
			while sWash.Menu do
				RageUIv1.IsVisible(RMenuv1:Get('wash', 'main'), true, true, true, function()
					RageUIv1.ButtonWithStyle("Laver votre véhicule", nil, {RightLabel = "~g~70 $~s~ →"}, true, function(h,a,s)
						if s then
							TriggerServerEvent('GreenLifePass:taskCountAdd:standart', 3 , 1)
...
AddEventHandler('es_carwash:success', function (price)
	...
	es_carwash_DrawNotification("Votre véhicule a été ~g~nettoyé~s~ ! \n- Vous avez payé ~g~" .. price .. " $~s~!")
end)
...
AddEventHandler('es_carwash:free', function ()
	WashDecalsFromVehicle(GetVehiclePedIsUsing(PlayerPedId()), 1.0)
	SetVehicleDirtLevel(GetVehiclePedIsUsing(PlayerPedId()))
	es_carwash_DrawNotification("Votre véhicule a été ~g~nettoyé~s~ gratuitement ~!")
end)
