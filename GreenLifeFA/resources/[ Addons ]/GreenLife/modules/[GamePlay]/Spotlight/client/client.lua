...
local maxtargetdistance = 150 -- max distance at which target lock is maintained
				...
				if IsHeliHighEnough(heli) then
					...
					if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
						if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
							...
							AddTextComponentString("~r~On ne peut pas faire de rappel de ce siège")
...
function RenderVehicleInfo(vehicle)
	if DoesEntityExist(vehicle) then
		...
		SetTextEntry("STRING")
		if vehicle_display == 0 then
			AddTextComponentString("Vitesse: " .. math.ceil(vehspeed) .. " " .. speed_measure .. "\nModèle: " .. vehname .. "\nPlaque: " .. licenseplate)
		elseif vehicle_display == 1 then
