...
CreateThread(function()
  if Bras.showBlipOnMap then
    for _, info in pairs(Bras.props) do
      info.blip = AddBlipForCoord(info.x, info.y, info.z)
      SetBlipSprite(info.blip, Bras.blip.id)
      SetBlipDisplay(info.blip, 4)
      SetBlipScale(info.blip, 0.5)
      SetBlipColour(info.blip, Bras.blip.colour)
      SetBlipAsShortRange(info.blip, true)
    BeginTextCommandSetBlipName("STRING")
      AddTextComponentString(Bras.blip.title)
...
CreateThread(function()
    while true do
      Wait(5000)
      for i, modelConfig in pairs(Bras.props) do
  ...
  Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(0, 51) and place == 0 then
          checkFunction()
        else
          Wait(500)
        end
  ...
  AddEventHandler('evy_arm:start_cl', function()
    ...
    if place == 1 then
      ...
      while grade >= 0.10 and grade <= 0.90 do
        Wait(3)
        PlayFacialAnim(PlayerPedId(), "electrocuted_1", "facials@gen_male@base")
        alert(text[Bras.language]['tuto'] .. "~INPUT_PICKUP~")
        SetEntityAnimSpeed(PlayerPedId(), "mini@arm_wrestling", "sweep_a", 0.0)
        SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a", grade)
  ...
  AddEventHandler('evy_arm:check_cl', function(args)
    ...
    elseif args == 'noplace' then
      ...
      notify('~r~' ..text[Bras.language]['full'])
