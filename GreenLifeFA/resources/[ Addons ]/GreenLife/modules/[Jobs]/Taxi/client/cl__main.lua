...
local function initPlayer()
    ...
    while playerData ~= nil and playerData.job ~= nil and playerData.job.name == "taxi" do
        ...
        elseif (#(playerCoords-Taxi.Config.vehicle.menuPosition) < 10.0) then
            loopInterval = 0
            DrawMarker(Config.Get.Marker.Type, Taxi.Config.vehicle.menuPosition, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], 30, 255, 0, 170, 0, 1, 0, 0, nil, nil, 0)
        end
        ...
        for i = 1, #storePosition do
            ...
            if (currentPosition ~= nil) then
                ...
                if (currentPedVehicle ~= 0 and DoesEntityExist(currentPedVehicle) and GetEntityModel(currentPedVehicle) == Taxi.Config.vehicle.model[GetEntityModel(currentPedVehicle)].model and #(playerCoords-currentPosition) < 2.0) then
                    loopInterval = 0
                ...
                elseif (currentPedVehicle ~= 0 and DoesEntityExist(currentPedVehicle) and GetEntityModel(currentPedVehicle) == Taxi.Config.vehicle.model[GetEntityModel(currentPedVehicle)].model and #(playerCoords-currentPosition) < 10.0) then
                    loopInterval = 0
                    DrawMarker(Config.Get.Marker.Type, currentPosition, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], 30, 255, 0, 170, 0, 1, 0, 0, nil, nil, 0)
                end
            end
        end
...
RegisterNetEvent("Taxi:loadConfig", function(taxiConfig)
    ...
    if (enterpriseData ~= nil) then
        ...
        AddTextComponentSubstringPlayerName("[Entreprise]",enterpriseData.label)
