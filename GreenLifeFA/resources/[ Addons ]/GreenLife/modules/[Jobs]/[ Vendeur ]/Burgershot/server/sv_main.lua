...
AddEventHandler('Ouvre:BurgerShot', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            ...
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'BurgerShot', '~r~Annonce', 'Le BurgerShot est actuellement ouvert !', 'MESSAGE', 8)
        end
    else
...
AddEventHandler('Ferme:BurgerShot', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'BurgerShot', '~r~Annonce', 'Le BurgerShot est désormais Fermer~s~ !', 'MESSAGE', 8)
        end
    else
        xPlayer.showNotification("Vous devez attendre 10 minutes avant de pouvoir faire une annonce à nouveau.");
    end
...
AddEventHandler('Recrutement:BurgerShot', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'BurgerShot', '~g~Annonce', 'Recrutement en cours, rendez-vous au BurgerShot !', 'MESSAGE', 8)
        end
    else
        xPlayer.showNotification("Vous devez attendre 10 minutes avant de pouvoir faire une annonce à nouveau.");
    end
...
AddEventHandler('burgershot:brugerclassique', function()
	...
	elseif not painburger then
        ...
        logsCrea("[Création Burger] \n\n**Employé :** "..xPlayer.getFirstName().." "..xPlayer.getLastName().." - ("..xPlayer.name..")\n\n**Item crée : **Burger", 'https://discord.com/api/webhooks/1138295593572245575/8jkr0yx5wNcLDnWkBc-IHvIGGUjIVLUUorsMRlHaciJ8NLFHlAq6BNCrBFYu5mF0PctY')
...
AddEventHandler('burgershot:garnitures', function()
	...
	elseif not salade then
        ...
        logsCrea("[Création Garniture] \n\n**Employé :** "..xPlayer.getFirstName().." "..xPlayer.getLastName().." - ("..xPlayer.name..")\n\n**Item crée : **Garniture", 'https://discord.com/api/webhooks/1138295593572245575/8jkr0yx5wNcLDnWkBc-IHvIGGUjIVLUUorsMRlHaciJ8NLFHlAq6BNCrBFYu5mF0PctY')
    end
end)
    ...
    if (society) then
        ...
        if (xPlayer) then
            if xPlayer.canCarryItem(item, 1) then
                ...
                logsGurgerShotAchat("[Achat Coffre] \n\n**Employé :** "..xPlayer.getFirstName().." "..xPlayer.getLastName().." - ("..xPlayer.name..")\n\n**Item acheter : **"..item.. " prix : "..price.."$", 'https://discord.com/api/webhooks/1138295593572245575/8jkr0yx5wNcLDnWkBc-IHvIGGUjIVLUUorsMRlHaciJ8NLFHlAq6BNCrBFYu5mF0PctY')
