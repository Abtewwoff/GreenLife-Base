		...
		while open do 
		   RageUI.IsVisible(mainMenu8,function() 
			...
			if ServiceBurgerShot then
                ...
                RageUI.Separator("~g~Vous devez être en service") 
			end
    ...
    while true do
        ...
        if ESX.PlayerData.job.name == 'burgershot' then
			...
			if #(coords - v.clothes) <= 10 then
				DrawMarker(20, v.clothes, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], 30, 255, 0, 170, 0, 1, 0, 0, nil, nil, 0)
				interval = 0
