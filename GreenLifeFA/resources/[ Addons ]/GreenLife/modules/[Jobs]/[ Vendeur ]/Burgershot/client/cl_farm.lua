...
local bfarm = RageUI.CreateMenu("", "BurgerShot Recolte", 0, 0, 'menustyle', 'interaction_bgd')
local btfarm = RageUI.CreateMenu("", "BurgerShot Traitement", 0, 0, 'menustyle', 'interaction_bgd')
local bvente = RageUI.CreateMenu("", "BurgerShot Vente", 0, 0, 'menustyle', 'interaction_bgd')
local bmveh = RageUI.CreateMenu("", "BurgerShot Sortie véhicule", 0, 0, 'menustyle', 'interaction_bgd')
                ...
                RageUI.IsVisible(bmveh, function()
                    for k, v in pairs(ConfigBurgerShotFarm.Veh) do
                        ...
                        RageUI.Button(v.label, v.description, { RightLabel = "→" }, vente == false, {
                            onSelected = function()
                                if via == false then
                                    ESX.ShowNotification("La place de parking est ~r~encombrer")
                                else
                    ...
                    { RightLabel = "→" }, true, {
                        onSelected = function()
                            local ped = PlayerPedId()
                            local vh = GetVehiclePedIsIn(ped, false)
                            local vhp = ESX.Game.GetVehicleProperties(vh)
                            
                            if vh ~= 0 then
                                if vhp.modLivery and vhp.plateIndex and vhp.windowTint == 2 then
                                    DeleteEntity(vh)
                                    vente = false
                                    RageUI.CloseAll()
                                else
                                    ESX.ShowNotification("Votre véhicule n'est pas celui du BurgerShot")
                                end
                            else
                                ESX.ShowNotification("Vous n'avez pas de véhicule")
                            end
                        end
                    })
                end)
    ...
    while true do
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == "burgershot" then
            ...
            if dif > 30 then
                ...
                if dif <= 3 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder a la ~g~récolte.")
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == "burgershot" then
            if dif > 20 then
                ...
                if dif <= 3 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~g~ traitement.")
                    if IsControlJustPressed(0, 51) then
                        bmtfarm()
                    end
                end
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == "burgershot" then
            if dif > 20 then
                ...
                ConfigBurgerShotFarm.Traitement.markercolor[3], 155)
                if dif <= 3 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder a la ~g~vente.")
                    if IsControlJustPressed(0, 51) then
                        bsell()
                    end
                end
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == "burgershot" then
            if dif > 20 then
                ...
                ConfigBurgerShotFarm.Traitement.markercolor[3], 155)
                if dif <= 5 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder a la ~g~sortie véhicule.")
                    if IsControlJustPressed(0, 51) then
                        bveh()
                    end
                end
