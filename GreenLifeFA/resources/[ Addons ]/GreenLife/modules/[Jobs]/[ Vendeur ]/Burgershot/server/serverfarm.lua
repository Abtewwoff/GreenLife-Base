    ...
    if xPlayer.getJob().name == "burgershot" then 
        if dist >= 10 then
            ...
            if xPlayer.canCarryItem(item, 1) then
                ...
                Wait(1000)
                TriggerClientEvent("ShowNotif", src, "~r~INFO : Pour arrêter la récolte veuillez sortir de la zone", 9000, src)
        ...
        if dist >= 10 then
            ...
            if verif then
                if xPlayer.canCarryItem(item, 1) then
                    ...
                    Wait(1000)
                    --print(dist)
                    TriggerClientEvent("ShowNotif", src, "~r~INFO : Pour arrêter le traitement veuillez sortir de la zone",
                    9000, src)
    ...
    if xPlayer.getJob().name == "burgershot" then 
        if dist >= 10 then
            ...
            if itemget then
                ...
                4000, src)
                Wait(1000)
                TriggerClientEvent("ShowNotif", src, "~r~INFO : Pour arrêter la vente veuillez sortir de la zone", 9000, src)
                Wait(5000)
                vente(src)
            else
                xPlayer.showNotification("~r~Vous n'avez plus assez d'items sur vous !")
                TriggerClientEvent("sverif", src)
                return
            end
        end
