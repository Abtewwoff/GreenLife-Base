...
AddEventHandler('Ouvre:realestateagent', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            ...
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Agent Immobilier', '', 'L\'Agence immobilier est actuellement ~g~ouvert ~s~ !', 'CHAR_IMMO', 8)
        end
    else
...
AddEventHandler('Ferme:realestateagent', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Agent Immo', '', 'L\'Agence immobilier est désormais  ~r~fermer~s~', 'CHAR_IMMO', 8)
        end
    else
        xPlayer.showNotification("Vous devez attendre 10 minutes avant de pouvoir faire une annonce à nouveau.");
    end
...
AddEventHandler('Recrutement:realestateagent', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Agent Immo', '', 'Recrutement en cours, rendez-vous a l\'Agence immobilier !', 'CHAR_IMMO', 8)
        end
    else
        xPlayer.showNotification("Vous devez attendre 10 minutes avant de pouvoir faire une annonce à nouveau.");
    end
