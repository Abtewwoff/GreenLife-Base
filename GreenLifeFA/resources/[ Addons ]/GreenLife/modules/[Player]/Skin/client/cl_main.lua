...
function ClothesSkinRota()
    RUIText({message = "Vous pouvez appuyer sur ~g~ESPACE~s~ pour tourner votre personnage."})             
                            ...
                            if v.table[1] ~= nil then
                                RageUI.List(v.label, v.table, i[v.name], nil, {}, true, {
                                    onSelected = function()
                                        ...
                                        FreezeEntityPosition(PlayerPedId(), false)
                                        ESX.ShowAdvancedNotification("Notification", "GreenLife", "Votre personnage a été sauvegarder avec succès !", "CHAR_ST", 1)
                                    end,
                                    onListChange = function(index, item)
                                        if index ~= nil then
                                            refreshDataSkin()
