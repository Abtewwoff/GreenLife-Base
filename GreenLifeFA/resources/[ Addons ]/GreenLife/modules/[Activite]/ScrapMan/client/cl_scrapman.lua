...
local interactionCooldown = 5000 -- 5 seconds cooldown for interactions
local lastInteractionTime = 0 -- Track the last interaction time
   ...
   while true do
      ...
      for k in pairs(Scrappos) do
         ...
         if (dist <= 10) then
            Interval = 0
            if not InJob then
               DrawMarker(1, Scrappos[k].x, Scrappos[k].y, Scrappos[k].z, 0, 0, 0, 0, 0, 0, 1.001, 1.0001, 0.2001, 30, 255, 0,135 ,0 ,0 ,0 ,0)

               if dist <= 1.2 then
                  -- TriggerEvent("ui:showInteraction", "E", " pour prendre de la feraille")
                  if IsControlJustPressed(0,38) and (GetGameTimer() - lastInteractionTime >= interactionCooldown) then
                     scrap()
                     exports.rprogress:Start('', harvestDuration)
                     InJob = true
                     lastInteractionTime = GetGameTimer() -- Update last interaction time
                  end
               end
            end
         end
      ...
      for k in pairs(Scrapsell) do
         ...
         if (dist <= 10) then
            Interval = 0
            if InJob then
               DrawMarker(1, Scrapsell[k].x, Scrapsell[k].y, Scrapsell[k].z, 0, 0, 0, 0, 0, 0, 1.001, 1.0001, 0.2001, 50, 205, 50, 255 ,0 ,0 ,0 ,0)
               if dist <= 1.2 then
                  -- TriggerEvent("ui:showInteraction", "E", " pour interagir avec l'action")
                  if IsControlJustPressed(0,38) and (GetGameTimer() - lastInteractionTime >= interactionCooldown) then
   ...
   CreateThread(function()
      ...
      while impacts < 3 do
         ...
         if impacts == 3 then
            ...
            ESX.ShowNotification("🛠️ Vous avez trouvé un type de ferraille, allez-y pour vendre cette ferraille au revendeur à proximité")
            break
         end
      end
