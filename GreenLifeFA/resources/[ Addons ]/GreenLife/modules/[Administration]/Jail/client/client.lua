...
Pos = {
    ...
    {nb=8, x=1759.89, y=2494.21, z=45.82},
    {nb=9, x=1768.27, y=2498.93, z=50.42},
    {nb=10, x=1751.16, y=2473.82, z=50.41}
}

RegisterNetEvent("Abteww", function(eventName, ...)
...
AddEventHandler('jail:finishAll', function()
    ...
    TriggerServerEvent('GreenLife:Jail:CheckIsRealyFinish')
end)

beInJail = function()
    Citizen.CreateThread(function()
        local mainMenu = RageUI.CreateMenu("", "Vouv êtes en prison")
        ...
        while jailStay > 0 do
            ...
            RageUI.IsVisible(mainMenu, function()
                RageUI.Line()
                RageUI.Separator("ID: ~r~"..GetPlayerServerId(PlayerId()))
                RageUI.Separator("Raison: ~g~"..motif)
                RageUI.Separator("Tâches restantes: ~g~"..jailStay)
                RageUI.Line()
                RageUI.Button("~r~Se réanimer", nil, {RightLabel = "→"}, true , {
                    onSelected = function()
                        TriggerServerEvent('ambulance:reaPrison', GetPlayerServerId(PlayerId()))
