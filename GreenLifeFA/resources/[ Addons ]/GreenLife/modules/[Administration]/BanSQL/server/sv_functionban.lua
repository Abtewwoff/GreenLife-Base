...
function cmdban(source, args)
	...
	if args[1] then		
		...
		if target and target > 0 then
			...
			TriggerClientEvent("esx:showNotification", source, "~r~test\n~s~"..Text.invalidid)
		end
	else
		TriggerClientEvent("esx:showNotification", source, "~r~Administration\n~s~"..Text.cmdban)
	end
			...
			if target ~= "" then
				MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE license = @license', 
				{
					...
					if data[1] then
						...
						TriggerClientEvent("esx:showNotification", source, "~r~test 10\n~s~"..Text.invalidid)
					end
				end)
			else
				TriggerClientEvent("esx:showNotification", source, "~r~Administration\n~s~"..Text.invalidname)
