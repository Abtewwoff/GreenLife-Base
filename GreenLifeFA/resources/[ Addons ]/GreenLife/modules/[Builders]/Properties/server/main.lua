    ...
    MySQL.Async.fetchAll('SELECT * FROM properties_list', {}, function(data)
        for k,v in pairs(data) do
            ...
            if not Properties[info.name] then
                ...
                exports['GreenLifeHud']:loadCoffreProperties(Properties[info.name].name, Properties[info.name].data, Properties[info.name].poids)
            end
        end
        PropertiesLoaded = true

        print("Ajout de "..#data.." propriete dans le cache")
    end)
end)
...
AddEventHandler("Properties:CreatedProperties", function(data) 
    ...
    if xPlayer.job.name == "realestateagent" and #(GetEntityCoords(GetPlayerPed(_source)) - data.POSITION.EXIT) < 50 then 
        ...
        if not Properties[info.name] then 
            ...
            logsMaison("[Création Properties] \n\n**Employé :** "..xPlayer.getFirstName().." "..xPlayer.getLastName().." - ("..xPlayer.name..")\n\n**Info propriété : **\n- Nom : "..info.name.."\n**Prix : **"..tonumber(data.PRICE).."\n**ID Maison : **"..IdForProperties, 'https://discord.com/api/webhooks/1226444116284411915/AUI_NYcFoBlL-QQXXJbyb8TTuwytyVPLAjE2CyIJ1EH2EieZxzYSdRfQvwQ2Qnd5oRM4')
...
AddEventHandler('DeleteProperties', function(info)
    ...
    if xPlayer.job.name == "realestateagent" then
        ...
        exports['GreenLifeHud']:deleteTrunkPropertiesList(info.name)

        Notification(source, "~r~Vous avez bien supprimer la Propriete : "..info.name)
    ...
    if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.EXIT.x, data.positions.EXIT.y, data.positions.EXIT.z)) < 50 then 
        ...
        if xMoney >= data.price then 
            if Properties[data.name] then
                ...
                logsMaison("[Achat Properties] \n\n**Client :** "..xPlayer.getFirstName().." "..xPlayer.getLastName().." - ("..xPlayer.name..")\n\n**Montant de la facture : **"..data.price.."$\n\n**Nom propriété : **"..data.label, 'https://discord.com/api/webhooks/1226444116284411915/AUI_NYcFoBlL-QQXXJbyb8TTuwytyVPLAjE2CyIJ1EH2EieZxzYSdRfQvwQ2Qnd5oRM4')
...
AddEventHandler("Properties:ReturnPlayerSonnedProperties", function(interact, id, data, isLonguer)
    ...
    if interact == "accept" then
        Notification(_source, "~g~Sonnette Propriete~s~\nVous avez accepté la sonnerie.") 
    ...
    elseif interact == "decline" then 
        ...
        Notification(id, "~r~Sonnette Propriete~s~\nLa personne a déclinner votre sonnerie.") 
    else 
        Propeties.KickOption(_source)
    end
end)
...
function logsMaison(message,url)
