...
Citizen.CreateThread(function()
	...
	RegisterNetEvent('JobBuilder:recolte', function(item)
		local source = source
	...
	RegisterNetEvent('JobBuilder:processing', function(itemProcessing, itemReward)
		local source = source
		local xPlayer = ESX.GetPlayerFromId(source)
		...
		if (jobs[xPlayer.job.name]) then
			...
			if xItem then
				...
				TriggerClientEvent("esx:showNotification", source, '~r~Vous n\'avez pas assez de '..itemProcessing..' pour faire cela.')
			end

		end
	end)

RegisterNetEvent('JobBuilder:sell', function(item, reward, societyName, societyLabel)
	...
	if (society) then
		if (jobs[xPlayer.job.name]) then
			if (xItem) then
				...
				TriggerClientEvent("esx:showNotification", source, '~r~Vous n\'avez pas assez de '..item..' pour faire cela.');
			end
		end
	else
		xPlayer.showNotification(("~r~Une erreur est survenue, Code Erreur: ~g~'job_builder_sell_error_society_not_found_%s'~s~. Veuillez contacter un administrateur."):format(societyName));
	end
end);
	...
	AddEventHandler('JobBuilder:withdrawMoney', function(societyName, amount)
		...
		if (society) then
			if amount > 0 and ESX.GetSocietyMoney(societyName) >= amount then
				...
				SendLogs("Entreprise", "GreenLife | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a retiré "..amount.."$ dans le coffre de l'entreprise "..society.."", "https://discord.com/api/webhooks/1226442772131745812/ijgxzBBdprb2JJASIN_Br0zXfTiUmHaLlK59kBR9ek3vHYHCuY0wfb2TEsBwrEY4WuhJ")
			else
			...
			if amount > 0 then
				if xPlayer.getAccount('cash').money >= amount then
					...
					if (society) then
						...
						SendLogs("Entreprise", "GreenLife | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a déposé "..amount.."$ dans le coffre de l'entreprise "..society.."", "https://discord.com/api/webhooks/1226442772131745812/ijgxzBBdprb2JJASIN_Br0zXfTiUmHaLlK59kBR9ek3vHYHCuY0wfb2TEsBwrEY4WuhJ")
					end
	...
	AddEventHandler('JobBuilder:Bossvirer', function(target)
		...
		if sourceJob.grade_name == 'boss' then
			...
			if sourceJob.name == targetJob.name then
				...
				TriggerClientEvent('esx:showNotification', sourceXPlayer.source, ('Vous avez ~r~viré %s.'):format(targetXPlayer.name))
				TriggerClientEvent('esx:showNotification', targetXPlayer.source, ('Vous avez été ~g~viré par %s.'):format(sourceXPlayer.name))
		...
		if sourceJob.grade_name == 'boss' then
			...
			if sourceJob.name == targetJob.name then
				...
				if newGrade ~= sourceJob.grade then
					...
					TriggerClientEvent('esx:showNotification', _src, ('Vous avez ~g~promu %s.'):format(targetXPlayer.name))
					TriggerClientEvent('esx:showNotification', target.source, ('Vous avez été ~g~promu par %s.'):format(sourceXPlayer.name))
					JobBuilderLogs("[Entreprise Payante] "..sourceXPlayer.name.." a promu "..targetXPlayer.name.." de l'entreprise "..sourceJob.name.."")
				else
					TriggerClientEvent('esx:showNotification', _src, 'Vous devez demander une autorisation ~r~Gouvernementale.')
				end
			else
				TriggerClientEvent('esx:showNotification', _src, 'Le joueur n\'es pas dans votre entreprise.')
			end
		else
			TriggerClientEvent('esx:showNotification', _src, 'Vous n\'avez pas ~r~l\'autorisation.')
		end
	end)
		...
		if sourceJob.grade_name == 'boss' then
			...
			if sourceJob.name == targetJob.name then
				...
				if newGrade >= 0 then
					...
					TriggerClientEvent('esx:showNotification', _src, ('Vous avez ~r~rétrogradé %s.'):format(targetXPlayer.name))
					TriggerClientEvent('esx:showNotification', target, ('Vous avez été ~r~rétrogradé par %s.'):format(sourceXPlayer.name))
					JobBuilderLogs("[Entreprise Payante] "..sourceXPlayer.name.." a rétrogradé "..targetXPlayer.name.." de l'entreprise "..sourceJob.name.."")
				else
					TriggerClientEvent('esx:showNotification', _src, 'Vous ne pouvez pas ~r~rétrograder d\'avantage.')
				end
			else
				TriggerClientEvent('esx:showNotification', _src, 'Le joueur n\'es pas dans votre entreprise.')
			end
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'society_'..society, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if (xPlayer.canCarryItem(itemName, count)) then
					...
					TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', '~g~Informations~s~', 'Vous avez retiré ~g~'..inventoryItem.label.." x"..count, 'CHAR_KIRINSPECTEUR', 8)
					...
					SendLogs("Entreprise", "GreenLife | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a retiré x"..count.." "..inventoryItem.label.." dans le coffre de l'entreprise "..society.."", "https://discord.com/api/webhooks/1226442772131745812/ijgxzBBdprb2JJASIN_Br0zXfTiUmHaLlK59kBR9ek3vHYHCuY0wfb2TEsBwrEY4WuhJ")
				else
					TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', '~g~Informations~s~', "Vous n'avez pas assez de place", 'CHAR_KIRINSPECTEUR', 9)
				end
			else
				TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', '~g~Informations~s~', "Quantité ~r~invalide", 'CHAR_KIRINSPECTEUR', 9)
			end
		end)
	end)
	...
	AddEventHandler('JobBuilder:putStockItems', function(itemName, count, society)
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'society_'..society, function(inventory)
			...
			if sourceItem then
				...
				TriggerClientEvent('esx:showAdvancedNotification', _src, 'Coffre', '~g~Informations~s~', 'Vous avez déposé ~g~'..inventoryItem.label.." x"..count, 'CHAR_KIRINSPECTEUR', 8)
				...
				SendLogs("Entreprise", "GreenLife | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a déposé x"..count.." "..inventoryItem.label.." dans le coffre de l'entreprise "..society.."", "https://discord.com/api/webhooks/1226442772131745812/ijgxzBBdprb2JJASIN_Br0zXfTiUmHaLlK59kBR9ek3vHYHCuY0wfb2TEsBwrEY4WuhJ")
			else
				TriggerClientEvent('esx:showAdvancedNotification', _src, 'Coffre', '~g~Informations~s~', "Quantité ~r~invalide", 'CHAR_KIRINSPECTEUR', 9)
			end
		end)
	end)
...
function JobBuilderLogs(message)
    local DiscordWebHook = "https://discord.com/api/webhooks/1226442772131745812/ijgxzBBdprb2JJASIN_Br0zXfTiUmHaLlK59kBR9ek3vHYHCuY0wfb2TEsBwrEY4WuhJ"
    local embeds = {
        {
            ["title"]=message,
            ["type"]="rich",
