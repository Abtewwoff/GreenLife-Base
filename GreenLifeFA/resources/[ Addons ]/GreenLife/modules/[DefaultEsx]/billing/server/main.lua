			...
			if xPlayer.job.name == sharedAccountName then
				MySQL.Async.fetchAll('SELECT * FROM billing WHERE identifier = @a', {
					...
					MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
					{
						...
						local link = "https://discord.com/api/webhooks/1226444500717670480/VuXAqLwqEJluT2FnswQbOWeWvOBDFqNLB4Xe58AJrhPlOV-3WYfRj84cgXDNi8ScHYuQ"
					...
					MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
						...
						if xPlayer.getVIP() == 3 then 
							if target == "cardealer" or target == "police" then
								...
								TriggerClientEvent('GreenLifePass:addtaskcount:standart', xPlayer.source, 6, 1)
...
function sendWebhookforbill(name,message,color)
    local local_date = os.date('%H:%M:%S', os.time())
    local DiscordWebHook = 'https://discord.com/api/webhooks/1226444500717670480/VuXAqLwqEJluT2FnswQbOWeWvOBDFqNLB4Xe58AJrhPlOV-3WYfRj84cgXDNi8ScHYuQ'
