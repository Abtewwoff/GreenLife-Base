...
CreateThread(function()
	while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Wait(3) end
    while not NetworkIsSessionStarted() or ESX.GetPlayerData().job == nil do Wait(3) end
    while true do
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 1.5 then
                    ...
                    if IsControlJustReleased(0, 51) and not inTrunk then
                        ...
                        if not exports.GreenLife:IsInPorter() and not exports.GreenLife:IsInOtage() then
                            if lockStatus == 1 then
                                if DoesEntityExist(playerPed) then
                                    if not IsEntityAttached(playerPed) or GetDistanceBetweenCoords(GetEntityCoords(playerPed), GetEntityCoords(PlayerPedId()), true) >= 5.0 then
                                        ...
                                        Wait(1500)
                                        SetVehicleDoorShut(vehicle, 5)
                                        TriggerServerEvent('GreenLifePass:taskCountAdd:premium', 1, 1)
                                    else
...
loadDict = function(dict)
    while not HasAnimDictLoaded(dict) do Wait(3) RequestAnimDict(dict) end
end
