...
-- 			ESX.ShowAdvancedNotification('Notification', 'GreenLife', "Pourquoi tu saute autant petit lapin ?")
-- 			resetCounter = 0
-- 		end
	...
	if closestDistance ~= -1 and closestDistance <= 3 then
		...
		if not (SafeZone:playerIsIn())then
			if not (IsCuffed) then
				if not piggyBackInProgress and not exports.GreenLife:IsInTrunk() then
