local acces = false
local VdaPos = 0
local VdaMenu = RageUI.CreateMenu("", "Vendeur d'armes illégal", 0, 0, 'menustyle', 'interaction_bgd')
        ...
        CreateThread(function()
            while open do 
                ...
                RageUI.IsVisible(VdaMenu,function()
                    RageUI.Separator("Vda Niveau : ~g~"..lvl)
                    RageUI.Line()
                    if lvl == 1 then
                        RageUI.Separator("Armes en vente ")
                        RageUI.Line()
                        for k,v in pairs (ConfigVDA.WeaponSell) do
                            RageUI.Button(v.WeaponLabel, nil, {RightLabel = "~r~"..v.price.."$"}, true, {
                                onSelected = function()
                        ...
                        for k,v in pairs (ConfigVDA.ItemSell) do
                            RageUI.Button(v.ItemLabel, nil, {RightLabel = "~r~"..v.price.."$"}, true, {
                                onSelected = function()
        ...
        if dst < 5.0 then
            ...
            if dst < 1.0 then
                ...
                if not RageUI.Visible(VdaMenu) then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour parler au ~r~Vendeur d'armes")
                end
