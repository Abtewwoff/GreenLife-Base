...
local function DrawText3DApu(B6zKxgVs, O3_X, DVs8kf2w, vms5, M7, v3)
    local ihKb = M7 or 7
...
AddEventHandler('eSup:OnPedDeathRobbery', function(store)
    SetPedHealth(eSup.Peds[store], 0)
end)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                wait = 1
                                DrawText3DApu(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur [~r~X~s~] pour ~s~ramasser le sac~s~.", 12)
...
AddEventHandler('eSup:ResetPedDeath', function(i)
    ...
    Wait(250)
    eSup.Peds[i] = _CreatePedSup(eSup.Braquage[i].shopkeeper, eSup.Braquage[i].coords, eSup.Braquage[i].heading)
...
function _CreatePedSup(hash, coords, heading)
    RequestModel(hash)
...
AddEventHandler("initializePoliceBlip2", function(ApuIndex)
    CreateThread(function()
        ...
        ESX.ShowAdvancedNotification("Superette", 'Vol en cours', "Braquage d'épicerie en cours !", "CHAR_CALL911", 4)
        ESX.ShowNotification('~g~g~s~ Accepter ~r~X~s~ Refuser')
        while true do
            Wait(3)
        ...
        if IsPedArmed(player, 5) then
            ...
            if IsPlayerFreeAiming(PlayerId()) or IsPedInMeleeCombat(player) then
                ...
                for i = 1, #eSup.Peds do
                    -- print(HasEntityClearLosToEntityInFront(player, eSup.Peds[i], 19), IsPedDeadOrDying(eSup.Peds[i]))
                        ...
                        if not eSup.IsRobbing then
                            ...
                            if canRob == true then
                                ...
                                if chance >= 0 and chance <= 25 then
                                    ...
                                    BraqueApuYesOrNo = true
                                    eSup.IsRobbing = true
                                    PlaySoundFrontend(-1, "Object_Dropped_Remote", "GTAO_FM_Events_Soundset", 0)
                                    ActivatePhysics(eSup.Peds[i])
                                    eSup.BarUpdate = AddTimerBar("Menace :",{percentage = 0.0, bg = {217, 255, 0}, fg = {30, 255, 0,255}})
                                    ...
                                    if GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(eSup.Peds[i]), true) <= 7.5 then
                                        if not IsPedDeadOrDying(eSup.Peds[i]) then
                                            ...
                                            if BraqueApuYesOrNo then
                                                ...
                                                Wait(20000)
                                                BraqueApuYesOrNo = false
                                            else
                                                ESX.ShowNotification("~r~APU se fait déjà braquer")
                                            end
                                        end
                                    else
                                        ClearPedTasks(eSup.Peds[i])
                                ...
                                ESX.ShowNotification("~r~Cette superette a déjà été braqué, le vendeur n'a plus rien.")
                            end
                            -- elseif canRob == 'no_cops' then
                            --     local wait = GetGameTimer() + 5000
                            --     while wait >= GetGameTimer() do
                            --         Wait(3)
                            --         ESX.ShowNotification('~r~Gruppe6 viens de passer, les caisses sont vides.')
