				...
				if not holdingup then
					...
					if Vdist(pos, v.position) < 1.5 then
						if not incircle then
							DisplayHelpText("Appuyez sur E pour braquer " .. v.nameofbank)
						end
	...
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 706.669, -966.898, 30.413, true) <= 10 and not blip then
			...
			if GetDistanceBetweenCoords(coords, 706.669, -966.898, 30.413, true) < 1.0 then
				DisplayHelpText("Appuyez sur E pour vendre")
				if IsControlJustReleased(1, 51) then
					blip = true;
					TriggerServerEvent('ZxGang:ClaquetteChausette');
				end
			end
		end
	end
...
RegisterNetEvent('ZxGang:ClaquetteChausetteV2', function()
