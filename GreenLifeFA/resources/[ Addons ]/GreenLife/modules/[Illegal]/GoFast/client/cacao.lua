...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if #(pCoords - Config.GoFast.Pos) <= 3 then
			...
			-- TriggerEvent("ui:showInteraction", "E", " pour programmer un GoFast")
...
finishGoFast = function()
	...
	if plate == '60LYV000' then
		ESX.ShowAdvancedNotification("GoFast", "~b~Livraison GoFast", "laisse le véhicule se garrer tout seul.", "CHAR_ILLEGAL2", 8)
...
-- Ouverture de toute les portes
		...
		ESX.ShowAdvancedNotification("GreenLife", "~b~Livraison GoFast", "Calcule du butin en cours ...", "CHAR_ILLEGAL2", 8)
		FreezeEntityPosition(ped, true)
		Wait(6000)
		...
		ESX.ShowAdvancedNotification("GreenLife", "~b~Livraison GoFast", "Hein ? C'est quoi ça ? C'est pas la voiture du GoFast !", "CHAR_ILLEGAL2", 8)
    ...
    Citizen.CreateThread(function()
        while GoFastEnCours do
            ...
            if #(pCoords - Config.GoFast.Sell) <= 10 then
				...
				-- TriggerEvent("ui:showInteraction", "E", " pour conclure votre GoFast")
                if IsControlJustPressed(0, 51) then
                    finishGoFast()
                end
            end
